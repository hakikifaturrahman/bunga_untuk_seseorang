...
export default function Header() {
    return (
        <AnimatePresence>
            <header className='relative w-full h-screen flex flex-col gap-14 md:flex-row items-center overflow-hidden'>
                <motion.div
                    initial={{ opacity: 0, x: -50 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ duration: 1 }}
                    className='w-full h-1/3 md:w-1/2 md:h-full flex flex-col gap-2 justify-center pt-14 px-[5%]
                    z-[1] text-4xl md:text-7xl'>
                    <h1 className='font-bold'>Hi There,</h1>
                    <span className='text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-green-600 font-bold'>I'm {METADATA.authors.name}</span>
                    <span className='text-2xl'>I will help you make website responsive and high quality.</span>
                </motion.div>
                ...
                <motion.div
                    ...
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ duration: 1 }}
                    className='w-full md:w-1/2 h-1/2 md:h-full flex justify-center items-center relative'
                >
                    <svg className='w-full h-full md:w-1/2 z-[-1] fill-neutral-200 dark:fill-neutral-800 md:scale-[2]'
                        id="visual" viewBox="0 0 900 900" width="100" height="100" xmlns="http://www.w3.org/2000/svg" xlinkHref="http://www.w3.org/1999/xlink" version="1.1">
                        <g transform="translate(427.92601285821485 412.79585478676427)">
                    ...
                    <div className='absolute z-[1] w-fit h-full md:h-fit'>
                        <img src="/img/src/mobile.svg" alt="mobile" width={'100%'} height={'100%'} className='h-full max-h-[450px]' />
                    </div>
                </motion.div>

                <div className='absolute w-full h-full top-0 left-0 z-[-1] overflow-x-hidden'>
                    <svg className='md:w-full h-full fill-neutral-200 dark:fill-neutral-800'
                        id="visual" viewBox="0 0 960 540" xmlns="http://www.w3.org/2000/svg"
                        xlinkHref="http://www.w3.org/1999/xlink" version="1.1" height={'100%'}>
                        <g>
